...
class StartConditionsDeserializer {
  ...
  deserializeInventoryData(node) {
    ...
    return {
      ...
      contents: contents
        ? Array.from(contents.children).map((c) =>
            this.deserializeInventoryData(c)
          )
        : [],
    };
  }
  ...
  deserializeStartingConditions(xmlstring) {
    ...
    let startingConditions =
      parsed.getElementsByTagName("StartingConditions")[0].children;
    return Array.from(startingConditions).map((condition) => {
      return {
        ...
        playerInventory: Array.from(
          this.getChildByName(condition, "PlayerInventory").children
...
var app = new Vue({
    ...
    fileChanged: function (evt) {
      if (evt.target.files.length === 1) {
        ...
        r.onload = () => {
          ...
          app.setConditions(
            deserializer.deserializeStartingConditions(r.result)
          );
        };
        r.readAsText(f);
      }
  ...
  computed: {
    downloadURL: function () {
      ...
      return (
        "data:text/plain," +
        encodeURIComponent(serializer.serializeConditions(this.startConditions))
      );
    },
  },
});

fetch("startconditions.xml")
  .then((r) => r.text())
  .then((text) => {
    let deserializer = new StartConditionsDeserializer();
